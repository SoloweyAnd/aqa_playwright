...
export class EditCustomerPage extends SalesPortalPage {
  //Modals
  deleteCustomerModal = new DeleteModalPage(this.page);
  ...
  async getInputValues() {
    const [email, name, country, city, street, house, flat, phone, notes] =
      await Promise.all([
  ...
  async getFormErrors() {
    return {
      ...
      phone: (await this.phoneError.isVisible())
        ...
        : null,
      notes: (await this.notesError.isVisible())
        ? await this.notesError.innerText()
        : null,
    };
  }
}
